...
- **Note (flagged):** the exact Inara **market-event schema** (`getCommodityPrices` `eventData`) is PROVISIONAL — verify against live Inara before shipping the real query. The API **key is injected** (the consumer reads it from secrets; integrations stays decoupled). POSTs are **uncached** (`ttlMs:0` — auth + per-body); the §5.3 2 h market-reference cache needs body-aware keying (later refinement). Inara **app registration** (§9) is required for a real key.
- **Correction (2026-07-14 Phase-4 gate review):** this step delivers the client + `parseInaraCommodityPrices` parser only. The earlier note claimed `source='inara'` ingestion into `market_snapshots` was "wired by the Ledger (4.11)" — **it was not** (4.11 built the read-side ledger-service; no `inara`/`capi` writer exists). The end-to-end **keyed-source ingestion wiring** (settings key → client → scheduled fetch → sink → `market_snapshots`) for BOTH Inara and cAPI is a **deferred Phase-4 follow-up** — see the Phase-4 deferred-follow-ups note below. Not building a sink with no live producer avoids dead product code (operator non-negotiable).
...
- Status: [x] DONE
- Zone: GREEN
- Files: `packages/intelligence/src/outfitter/{templates.ts,advisor.ts}`, `packages/shared/src/outfitter.ts`, `apps/desktop/src/main/outfitter-wiring.ts`, `apps/desktop/src/renderer/components/outfitter/OutfitterPanel.tsx` + tests
- **DECOMPOSITION (2026-07-14, changelog-noted):** split into **4.15a** (pure `intelligence/outfitter` — templates + advisor) and **4.15b** (Outfitter UI). Same pattern.
- **4.15b notes (flagged):** the Outfitter is **not a nav module** — the §3.2 module list is fixed at 11 (operator-approval to change), so it ships as a **"Loadout Advisor" panel inside the Vein Finder** (the natural mine-target ⇄ equipment pairing; SSOT `screens/Outfitter.tsx` → `components/outfitter/OutfitterPanel.tsx` divergence noted). Main captures the last `Loadout` (ship + modules) via `engine.onEvent`. Ship-slot data isn't sourced yet → suggestions aren't slot-filtered (the advisor's fit-check is ready; wiring a ship-slot table is a follow-up).
...
**Phase 4 gate status (2026-07-14):** DoD **MET** for the core capability — the Vein Finder returns a scored, explained (full 4.5 term breakdown), filterable (commodity/distance/reserve/ring-type/pad) ranking with a best sell station, and the Cartographer produces a full copyable round-trip plan (plain system names, no injection). Live journal prices flow into `market_snapshots` (Phase-2 price-book) and are consumed end-to-end; **first-party now correctly outranks the spoofable EDDN firehose** (gate-review fix, below). Whole-phase adversarial review (architecture + design/correctness + red-team/compliance) run; Prime Directive clean (no game-input path anywhere in Phase 4, gateway-only egress to the §5.4 allowlist, EDDN spoof-hardened + advisory, seed clean-room with provenance).

**Phase 4 deferred follow-ups (surfaced for operator decision — NOT slipped in):**
1. **EDDN transport** — the ZeroMQ SUB adapter to `tcp://eddn.edcd.io:9500` + its native `zeromq` dep / `onlyBuiltDependencies` + egress-firewall socket-sanctioning (4.8 note). The sink is built + tested; the live socket producer awaits the operator's native-dep decision.
2. **Inara + cAPI keyed-source ingestion** — end-to-end wiring (settings key → client → scheduled fetch → sink → `market_snapshots`, `source='inara'`/`'capi'`) is unbuilt. The Inara client + price parser exist (4.9); the cAPI client exists (4.10, needs a `/market` parser). Gated behind live-schema verification and Frontier **§9 app registration** (cAPI). "(+ Inara/cAPI when keyed)" in the DoD is not yet exercisable until this + a real key land.
3. **4.14 3D `Canvas`** — react-three-fiber 3D map + manual fps benchmark (the 2D SVG schematic is the working, tested fallback).
4. **Live wiring gaps** — hotspot recorder → live engine subscription (4.3); alert evaluation → notification/TTS delivery (4.11); Spansh-backed `RouteProvider` injection (straight-line fallback in use, 4.12); station-distance joins that would activate the `distanceLs`/`maxSellLegLs` filters (4.11/4.13); "Plan this" exact-run pre-fill (4.13).
...
- 2026-07-14 — **Phase 4 gate review — fixes.** The whole-phase adversarial review (architecture + design/correctness + red-team/compliance) over the Phase-4 tree found the architecture and Prime-Directive/compliance surfaces CLEAN, and two blocking correctness findings, now fixed (TDD): **(1) first-party prices were silently out-ranked by EDDN.** The pure sell-ranking trust map (`intelligence/ledger/best-sell.ts` `DEFAULT_SELL_WEIGHTS`, v1→v2) keyed `journal`/`capi`, but the PRODUCTION journal writer (`core/market/price-book.ts`) stamps `market_snapshots.source` as `market` (Market.json on dock) / `marketsell` (MarketSell) — neither was in the map, so a commander's own dock price fell to `unknownSourceTrust:0.75`, **below** `eddn:0.8`, inverting the DoD/Step-4.11 guarantee that a spoofable EDDN firehose price never beats first-party. Added `market`/`marketsell` = 1.0 and regression tests using the REAL source strings (a `market` row outranks a HIGHER eddn headline: 500k×1.0 > 590k×0.8) at both the pure (`best-sell.test.ts`) and DB→adapter (`ledger-service.test.ts`) levels — the old tests masked it with the fictional string `journal` the ingestion never emits. **(2) honesty:** `veins/vein-service.ts` rendered an unparseable/missing `last_confirmed` as LIVE (`Date.parse(...) || nowMs`) — now falls back to epoch 0 (classifies STALE, IPC-safe where NaN→null), tested. **Documented (not slipped):** corrected the inaccurate 4.9 note that claimed the Inara `market_snapshots` sink was "wired by 4.11" (it was not — no `inara`/`capi` writer exists); consolidated the honest **Phase-4 deferred follow-ups** (EDDN ZeroMQ transport; Inara/cAPI keyed-source end-to-end ingestion wiring; 4.14 3D Canvas; live-engine/alert-delivery/Spansh-provider/station-distance wiring) next to the DoD for the operator gate. Building a sink with no live producer was deliberately NOT done (would be dead product code). lint + typecheck + compliance + full suite green.
- 2026-07-14 — **Step 4.15b done — Outfitter UI. STEP 4.15 COMPLETE — PHASE 4 FEATURE-COMPLETE.** `packages/shared/src/outfitter.ts` DTOs (`OutfitterModule`/`OutfitterGap`{category,minSize,reason,fitsShip}/`OutfitterAdvice`) + the `outfitter.advise` channel (ChannelPayloads + CHANNEL_SET + the exact-list channels.test all in sync, union → 31). `core` re-exports the 4.15a `analyzeLoadout` for the renderer (desktop can't import `intelligence`). Main: `outfitter-wiring.ts` (`createOutfitterBridge(getLoadout)`: runs `analyzeLoadout` on the last-captured `CapturedLoadout` for the chosen method → `OutfitterAdvice`; `hasLoadout:false` before any capture) + `index.ts` captures the last `Loadout` (ship + modules) via `engine.onEvent` + `ipc.ts` arg-validated `outfitter.advise` (method ∈ laser|deep-core|subsurface) + preload `adviseOutfit`. Renderer: `components/outfitter/OutfitterPanel.tsx` — the **"Loadout Advisor"** with a method picker, the gap analysis (equipped / **missing-required** with reason + `fitsShip` flag / **recommended**) and an explicit no-loadout first-run state; **rendered as a panel inside the Vein Finder** (what to mine ⇄ are you equipped for its method). Read-only — LODESTAR never changes the ship. 319 desktop tests (panel: missing-required+reason+recommendations, method-refetch, all-clear, unfittable flag, no-loadout, graceful-failure; bridge: no-loadout + per-method analysis; ipc: handler + arg-rejection). **Flagged (surfaced, not slipped):** the Outfitter is **not a nav module** — the §3.2 module list is fixed at 11 (operator-approval to change), so it ships as a Vein-Finder panel (SSOT `screens/Outfitter.tsx` → `components/outfitter/OutfitterPanel.tsx`); a ship-slot table isn't sourced yet → suggestions aren't slot-filtered (the advisor's fit-check is ready). lint + typecheck + compliance + full suite green.
